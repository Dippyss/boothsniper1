...
Booths_Broadcast.OnClientEvent:Connect(function(username, message)
		...
		elseif class == "Charm" and unitGems <= 1 then
		    if not string.find(item, "Coins") and not string.find(item, "Agility") and not string.find(item, "Bonus") then
                ...
                elseif class == "Egg" and unitGems <= 1000000 then
                    coroutine.wrap(tryPurchase)(uid, gems, item, version, shiny, amount, username, class, playerid, buytimestamp, listTimestamp, snipeNormal)
                    return
                elseif ((string.find(item, "Secret Key") and not string.find(item, "Lower")) or string.find(item, "Ticket")) and unitGems <= 50000 then 
                    coroutine.wrap(tryPurchase)(uid, gems, item, version, shiny, amount, username, class, playerid, buytimestamp, listTimestamp, snipeNormal)
                    return
                elseif ((string.find(item, "Crystal Key") and not string.find(item, "Lower")) or string.find(item, "Ticket")) and unitGems <= 15000 then 
                    coroutine.wrap(tryPurchase)(uid, gems, item, version, shiny, amount, username, class, playerid, buytimestamp, listTimestamp, snipeNormal)
                    return
